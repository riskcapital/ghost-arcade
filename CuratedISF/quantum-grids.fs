...
vec3 getColor(int index, float time) {
    ...
    float colorIndex = float(index % numColors);
...
vec4 drawTronFloor(vec2 uv, float time) {
    ...
    for (float i = 0.0; i <= depth + 2.0; i += 1.0) {
...
vec4 drawTronFullTunnel(vec2 uv, float time) {
    ...
    for (float i = 0.0; i <= numCornerLines; i += 1.0) {
...
vec4 drawConcentricSquaresStatic(vec2 uv, float time) {
    ...
    for (float i = 0.0; i <= ns + 2.0; i += 1.0) {
...
vec4 drawConcentricSquaresRotating(vec2 uv, float time) {
    ...
    for (float i = 0.0; i <= ns + 2.0; i += 1.0) {
        ...
        vec2 corners[4];
        corners[0] = rotation * vec2(-halfSize, -halfSize);
        corners[1] = rotation * vec2(halfSize, -halfSize);
        corners[2] = rotation * vec2(halfSize, halfSize);
        corners[3] = rotation * vec2(-halfSize, halfSize);

        for (int j = 0; j < 4; j++) {
            vec2 p1 = center + corners[j];
            vec2 p2 = center + corners[(j + 1) % 4];
            finalColor += drawGlowLine(uv, p1, p2, color, alpha, lineWeight);
        }
    }
...
vec4 drawConcentricCirclesStatic(vec2 uv, float time) {
    ...
    for (float i = 0.0; i <= ns + 5.0; i += 1.0) {
