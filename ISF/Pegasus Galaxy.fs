/*{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/lty3Rt by frankenburgh.  Galaxy shader from Pegasus 4k Intro\n4kb executable: http://www.pouet.net/prod.php?which=65859\nhttps://www.youtube.com/watch?v=3pXhImWoNsw",
    "IMPORTED": {
    },
    "INPUTS": [
        {
...
void main() {
	...
	float fade = 1.;//min(1., time*1.)*min(1.,max(0., 15.-time));
	// start glow after 5=50sec
	float fade2= 0.;//max(0., time-10.)*0.37;
	float glow = GLOW;//max(-.25,1.+pow(fade2, 10.) - 0.001*pow(fade2, 25.));
	
	
	// get camera position and view direction
	//vec3 campos = vec3(500.0, 850., -.0-cos((time-1.4)/2.)*2000.); // moving
	vec3 campos = vec3(500.0, 850., -2000.);
	campos *= vec3(CAMPOS_X, CAMPOS_Y, CAMPOS_Z); // moving
	...
	// stars
	vec3 stars = STARS*85.5*vec3(pow(fbmslow(rd.xyz*312.0), 7.0))*vec3(pow(fbmslow(rd.zxy*440.3), 8.0));
	
	// moving background fog
    vec3 cpos = 1500.*rd + vec3(831.0-time*30., 321.0, 1000.0)*FOG;
    col += vec3(0.4, 0.5, 1.0) * ((fbmslow( cpos*0.0035 ) - .5))*FOG;
	cpos += vec3(831.0-time*33., 321.0, 999.)*FOG;
    col += vec3(0.6, 0.3, 0.6) * 10.0*pow((fbmslow( cpos*0.0045 )), 10.0)*FOG;
	cpos += vec3(3831.0-time*39., 221.0, 999.0)*FOG;
    col += 0.03*vec3(0.6, 0.0, 0.0) * 10.0*pow((fbmslow( cpos*0.0145 )), 2.0)*FOG;
	// stars
    ...
    if (radius<1.8)
    {
  	  for (int q=10; q>-10; q--) // layers
      {
		...
		// border
 	    float radius = length(cpos.xz)/999. * RADIUS;
	    if (radius>1.0)
	      continue;
		...
		float gstar = 2.*pow(noise( cpos*21.40 ), 22.0*GSTAR);
		float gstar2= 3.*pow(noise( cpos*26.55 ), 34.0*GSTAR2);
		float gholes= 1.*pow(noise( cpos*11.55 ), 14.0*GHOLES);
    ...
    // Vignetting
	vec2 xy2 = gl_FragCoord.xy / RENDERSIZE.xy;
