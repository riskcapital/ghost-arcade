...
import { WebGPUVolumetricSpheresShader, type VolumetricSpheresParams } from './webgpuVolumetricSpheresShader';
...
export class WebGPUSmokeRidersShader implements GpuShaderImpl {
  ...
  private applyParams(): void {
    ...
    const bass = reactive ? Math.min(1.8, this.bands!.bass * Number(p.bassDrive ?? 1.2)) : 0;
    const treble = reactive ? this.bands!.treble : 0;
    const colorA = this.rgb01(p.colorA, [0.28, 0.66, 1.0]);
    const colorB = this.rgb01(p.colorB, [1.0, 0.28, 0.62]);
    const colorC = this.rgb01(p.colorC, [1.0, 0.8, 0.36]);
    const colorD = this.rgb01(p.colorD, [0.47, 1.0, 0.87]);
    const smokeTint = this.rgb01(p.smokeColor, [0.07, 0.09, 0.18]);
    const gridSize = quality === 'ultra' ? 64 : quality === 'performance' ? 32 : 48;
    const countScale = quality === 'ultra' ? 1.25 : quality === 'performance' ? 0.55 : 1;
    const styleTuning = this.styleTuning(style);

    this.smoke.setParams({
    ...
    } as Partial<VolumetricSpheresParams>);
  }
  ...
  private styleTuning(style: string): {
    emitters: number;
    spawnY: number;
    splatRadius: number;
