...
<script lang="ts">
  ...
  function bandLabel(band: RoutingBand): string {
    return $t(`liveVisuals.stems.${band}`);
  }
...
<div class="ss-panel">
  <div class="ss-header">
    <span class="ss-title">{$t('liveVisuals.stems.title')}</span>
    <span class="ss-sub">{$t('liveVisuals.stems.subtitle')}</span>
  </div>

  {#if !state.running}
    <div class="ss-row">
      <label class="ss-label">{$t('liveVisuals.stems.device')}</label>
      <select bind:value={pickedDeviceId} aria-label={$t('liveVisuals.stems.device')}>
        {#each devices as d}
          <option value={d.deviceId}>{d.label}{d.isVirtual ? '  ◆' : ''}</option>
        {/each}
        {#if devices.length === 0}<option value="">{$t('liveVisuals.stems.noInputs')}</option>{/if}
      </select>
    </div>
    <div class="ss-row">
      <label class="ss-label">{$t('liveVisuals.stems.layout')}</label>
      <select bind:value={pickedLayoutId} aria-label={$t('liveVisuals.stems.layout')}>
    ...
    <div class="ss-actions">
      <button class="ss-btn primary" onclick={startStems} disabled={starting || !pickedDeviceId}>
        {starting ? $t('liveVisuals.stems.starting') : $t('liveVisuals.stems.start')}
      </button>
      <button
        class="ss-btn small"
        onclick={refreshDevices}
        title={$t('liveVisuals.stems.rescanInputs')}
        aria-label={$t('liveVisuals.stems.rescanInputs')}>↻</button
      >
    </div>
    {#if state.error}
      <div class="ss-err">{$t('liveVisuals.stems.error', { values: { error: state.error ?? '' } })}</div>
    {/if}
    <div class="ss-hint">
      {$t('liveVisuals.stems.routeHint', { values: { devices: 'BlackHole, Loopback' } })}
      <strong>{$t('liveVisuals.stems.demucsLayout')}</strong>
      {$t('liveVisuals.stems.demucsHint')}
    </div>
  {:else}
    <div class="ss-active">
      <span class="ss-dot"></span>
      <span class="ss-active-name">{activeLayout?.name ?? $t('liveVisuals.stems.active')}</span>
      <button
        class="ss-btn small"
        onclick={stopStems}
        title={$t('liveVisuals.stems.stop')}
        aria-label={$t('liveVisuals.stems.stop')}>{$t('liveVisuals.stems.stop')}</button
      >
    </div>
    ...
    <div class="ss-matrix">
      <div class="ss-matrix-head">
        <span></span>
        <span>{$t('liveVisuals.stems.bass')}</span>
        <span>{$t('liveVisuals.stems.mid')}</span>
        <span>{$t('liveVisuals.stems.treb')}</span>
        <span>{$t('liveVisuals.stems.level')}</span>
      </div>
      {#each activeStems as stem (stem.id)}
        <div class="ss-matrix-row">
          <span
            class="ss-stem-label"
            title={$t('liveVisuals.stems.channels', {
              values: { channels: stem.channels.map((c) => c + 1).join('+') },
            })}>{stem.label}</span
          >
          {#each ['bass', 'mid', 'treb'] as RoutingBand[] as band}
            <input
              ...
              title={$t('liveVisuals.stems.cellTitle', {
                values: {
                  stem: stem.label,
                  band: bandLabel(band),
                  value: getCell(stem.id, band).toFixed(2),
                },
              })}
              aria-label={$t('liveVisuals.stems.cellTitle', {
                values: {
                  stem: stem.label,
                  band: bandLabel(band),
                  value: getCell(stem.id, band).toFixed(2),
