...
const liveCaptureNode = path.join(outputDir, 'live_capture_addon.node');
...
for (const slice of slices) {
  ...
  if (!fs.existsSync(liveCaptureNode)) {
    console.error(`[native-build] Expected build output missing: ${liveCaptureNode}`);
    process.exit(1);
  }
  ...
  fs.copyFileSync(liveCaptureNode, path.join(sliceDir, `live_capture_addon-${slice.lipoArch}.node`));
...
lipoSlices(liveCaptureNode, 'live_capture_addon', { required: true });
